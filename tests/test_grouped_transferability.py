...
def test_opposing_independent_groups_is_mixed_not_rescued_by_mean():
    ...
    result = score_independent_groups(
        model,
        [("stable", model.copy()), ("reversed", model[::-1].copy())],
        base_axes=(0, 1),
        added_axes=(2,),
    )
...
def test_group_mass_does_not_change_equal_group_decision_weighting():
    ...
    stable = model.copy()
    reversed_support = model[::-1].copy()
    scaled = score_independent_groups(
        model,
    ...
    assert scaled.gains == pytest.approx(equal_mass.gains)
    assert scaled.classification == equal_mass.classification == "mixed"
    assert scaled.equal_group_mean_gain == pytest.approx(equal_mass.equal_group_mean_gain)
...
def test_empty_duplicate_and_invalid_group_ids_fail_closed():
    ...
    with pytest.raises(ValueError, match="at least one"):
        score_independent_groups(model, [], base_axes=(0, 1), added_axes=(2,))
    with pytest.raises(ValueError, match="duplicate"):
        score_independent_groups(model, [("a", model), ("a", model)], base_axes=(0, 1), added_axes=(2,))
    with pytest.raises(ValueError, match="non-empty strings"):
        score_independent_groups(model, [(" ", model)], base_axes=(0, 1), added_axes=(2,))
...
def test_known_truth_grouped_transferability_benchmark_recovers_all_categories():
    ...
    assert checks
    assert {check.family for check in checks} == {"all_stable", "stable_plus_shifted", "all_shifted"}

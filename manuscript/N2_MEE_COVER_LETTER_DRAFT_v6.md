...
> Replace bracketed administrative placeholders only after the author team confirms them. The scientific claims below are bounded by the validated v6 evidence and should not be strengthened without new evidence.
...
We then execute two public-data state-prediction endpoints whose source archives, state bins, predictors, independent groups, model settings and terminal rules were frozen before outcome access. The first, using MH_ANTWERPEN marsh-harrier tracking, contained 193,370 thinned events but only three eligible independent individuals against a frozen minimum of four; it therefore closed as unavailable before transfer scoring. The second used the fixed BOP_RODENT v3 raptor archive and admitted 154,655 events from 30 individuals across four species. Random-forest state predictions improved held-out log score over the pooled training marginal for 27 of 30 individuals and improved multiclass Brier score for all 30. Under the predeclared all-individual rule, the prospective terminal result remains mixed because three individuals had non-positive primary gain.

Because species identity was itself a predictor in the BOP model, we retain the explicitly post-outcome decomposition of the already frozen primary gain. Across the 30 held-out individuals, the mean total gain of `0.57091` nats/event decomposes into a pooled-to-species baseline component of `0.07279` and a within-species context component of `0.49812`. This amendment did not refit the model, re-access raw tracking data, retune any setting, replace the prospective comparator or change the 27/30 mixed terminal decision.

Version 6 adds a second, explicitly post-outcome descriptive summary using those same frozen individual gains. Individuals receive equal weight and species is the resampling cluster. The total first-to-final gain averages `0.57091` nats/event with a 95% species-cluster bootstrap interval of `[0.23969, 1.04119]`; 27/30 individuals are positive and the cluster-bootstrap lower bound on that fraction is `0.793`. Heterogeneity remains visible: the labelled normal-theory new-individual prediction interval is `[-0.423, 1.565]`, although the empirical 10th percentile is `0.120`. The pooled-to-species adjacent increment remains uncertain, so the non-skippable stepwise population ceiling remains pooled even though total first-to-final transfer is positive. With only four represented species clusters, we treat this as descriptive rather than precise species-superpopulation inference. It does not reclassify the prospective mixed endpoint.

Earlier Tawaki, European free-tailed bat and Snapshot Serengeti analyses are retained as supporting diagnostics rather than relabelled as new prediction demonstrations. Together they show why estimability, added-state structure, independent transfer and population heterogeneity must remain distinct evidential layers.

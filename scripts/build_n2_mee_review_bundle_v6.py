...
def _set_review_package_version(stage: Path) -> None:
    path = stage / "pyproject.toml"
    text = path.read_text(encoding="utf-8")
    updated, count = re.subn(
        r'(?m)^version = "[^"]+" -> dict[str, object]:
...
def _scan_identity(stage: Path) -> None:
    ...
    if violations:
        raise ValueError(
            "anonymous bundle contains identity tokens: " + "; ".join(violations)
        )
...
`empirical_state_prediction_mixed`. A second post-outcome descriptive summary
uses the already frozen individual gains to report equal-weight population mean
transfer, positive-group prevalence, heterogeneity, and a labelled new-individual
prediction interval. Species is the resampling cluster. Total first-to-final
transfer is reported separately from the non-skippable adjacent-step ceiling.
With only four represented species clusters, the result is not treated as precise
species-superpopulation inference. No model was refit, raw tracking data were not
re-accessed, and this summary cannot reclassify the prospective endpoint. See
`review_evidence/BOP_POPULATION_TRANSFER.json` and
...
def _manifest(stage: Path) -> dict[str, object]:
    ...
    return {
        ...
        "contains_post_outcome_bop_population_transfer": True,
...
def build_bundle(output: Path) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="n2-review-v6-") as temp:
        ...
        _set_review_package_version(stage)
        ...
        evidence_path = (
            stage / "review_evidence" / "BOP_POPULATION_TRANSFER.json"
        )
    ...
    return {
        ...
        "python_file_ai_annotation_count": manifest[
            "python_file_ai_annotation_count"
        ],
    }
...
if __name__ == "__main__":
    main()
,
        f'version = "{REVIEW_PACKAGE_VERSION}"',
        text,
        count=1,
    )
    if count != 1:

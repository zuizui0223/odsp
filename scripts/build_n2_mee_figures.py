...
The script does not refit or rerun any empirical endpoint. It reads immutable
terminal/synthesis JSON files and renders manuscript displays only.
"""
...
def figure_data() -> dict[str, object]:
    ...
    bat_gains = [
        float(item["mean_log_score_gain"])
        for item in bat["primary"]["sealed_individual_scores"]
    ]
    serengeti_gains = [float(value) for value in serengeti["heldout_gains"]]
    ...
    return {
        ...
        "empirical": {
            ...
            "serengeti": {
                ...
                "gains": serengeti_gains,
                "terminal_category": str(serengeti["terminal_category"]),
            },
        },
        "matrix_chapter_claim": matrix["chapter_level_synthesis"]["strongest_supported_claim"],
    }
...
def build_figure_1(output_dir: Path) -> list[Path]:
    ...
    ax.text(5.25, 3.55, "Multidimensional support  S(B,A)  →  lower-dimensional projection  S(B)",
            ha="center", va="center", fontsize=11, weight="bold")
    ax.text(5.25, 0.45,
            "A positive result at one layer does not automatically satisfy the next layer.",
...
def build_figure_2(output_dir: Path, data: dict[str, object]) -> list[Path]:
    ...
    ax.set_title("Known-truth states separate fitted organization from transferability")
    ax.legend(frameon=False)
    ax.text(x[0], 0.055, "4 effective\nadded states", ha="center", va="bottom", fontsize=8.5)
    ax.text(x[2] + width / 2, gains[2] - 0.035, "organization shifts\nin held-out support",
            ha="center", va="top", fontsize=8.5)
    fig.tight_layout()
    return _save(fig, output_dir, "Figure2_known_truth_validation")
...
def build_figure_3(output_dir: Path, data: dict[str, object]) -> list[Path]:
    ...
    ax.text(0.03, 0.97,
            f"H(Z|X,Y) = {bat['information_nats']:.3f} nats\n"
            f"effective states = {bat['effective_states']:.2f}\n"
            "thick / non-generalizing",
    ...
    ax.text(0.03, 0.97,
            f"H(T|Site) = {serengeti['information_nats']:.3f} nats\n"
            f"effective states = {serengeti['effective_states']:.2f}/6\n"
            f"I(Species;T|Site) = {serengeti['partition_information_nats']:.3f}\n"
            f"permutation p = {serengeti['permutation_p_value']:.3f}\n"
            "thick / generalizing",
